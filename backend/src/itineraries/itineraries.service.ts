...
import { sortDays } from './itinerary.persistence';
...
export class ItinerariesService {
  ...
  create() {
    throw new ConflictException({
      ...
      message: 'Manual creation is disabled in auto-save mode',
    });
  }
  ...
  async list(userId: string, query: ListItinerariesQueryDto, pageSize = 10) {
    ...
    const items = rawItems.map((itinerary) => {
      const sunnyDays = itinerary.days.filter(
        (day) => day.scenario === DayScenario.SUNNY,
      );
      const primary = sunnyDays.length ? sunnyDays : itinerary.days;
      const firstDate = primary.length ? primary[0].date.toISOString() : null;
      const lastDate = primary.length
        ? primary[primary.length - 1].date.toISOString()
        : null;
      const activities = itinerary.days.flatMap((day) => day.activities ?? []);
      const areaRanking = buildAreaRanking(
        activities.map((activity) => activity.area),
      );
      const summary = buildDescriptionSummary(
  ...
  async update(id: string, dto: UpdateItineraryDto, userId: string) {
    ...
    if (itinerary.version !== dto.version) {
      throw new ConflictException({
        ...
        details: { currentVersion: itinerary.version },
      });
    }
    ...
    await this.prisma.$transaction(async (tx) => {
      await tx.itinerary.update({
        where: { id },
        data: { title: dto.title ?? itinerary.title, version: nextVersion },
      });
      ...
      if (dto.days) {
        ...
        if (existingDayIds.length) {
          await tx.activity.deleteMany({
            where: { itineraryDayId: { in: existingDayIds } },
          });
          await tx.itineraryDay.deleteMany({
            where: { id: { in: existingDayIds } },
          });
        }
        ...
        for (const day of sortedDays) {
          ...
          await tx.activity.createMany({
            data: day.activities.map((activity, idx) => ({
              ...
              category:
                (activity.category as SpotCategory) ?? SpotCategory.SIGHTSEEING,
